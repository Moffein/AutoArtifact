...
{
    [BepInPlugin("com.Moffein.AutoArtifact", "AutoArtifact", "1.1.1")]
    public class AutoArtifactPlugin : BaseUnityPlugin
    {
        ...
        private void SceneDirector_Start(On.RoR2.SceneDirector.orig_Start orig, SceneDirector self)
        {
            if (NetworkServer.active && Run.instance && RunArtifactManager.instance)
            {
                ...
                {
                    bool useStageCheck = info.stageClearCount >= 0;
                    bool meetsStages = !useStageCheck || Run.instance.stageClearCount >= info.stageClearCount;
                    ...
                    bool shouldEnable = meetsPlayers && meetsStages;
                    bool artifactEnabled = RunArtifactManager.instance.IsArtifactEnabled(info.artifactDef);

                    if (artifactEnabled != shouldEnable)
                    {
                        string actionString = shouldEnable ? "Enabled" : "Disabled";
                        Debug.Log("AutoArtifact: " + actionString + " " + info.artifactDef.cachedName + ", Meets StageClearCount Requirement: " + meetsStages + ", Meets PlayerCount Requirement: " + meetsPlayers);
                        RunArtifactManager.instance.SetArtifactEnabledServer(info.artifactDef, shouldEnable);
                    }
                }
            }
            orig(self);
        ...
        {
            artifactListString = base.Config.Bind<string>(new ConfigDefinition("Settings", "Artifact List"), "", new ConfigDescription("List of artifacts separated by commas. Format is ArtifactName:StageClearCount:PlayerCount (ex. Command, Honor:10, Sacrifice:-1:5). Use negative numbers to skip a check (ex. Sacrifice:-1:5 means that StageCount requirement will be ignored, meaning only PlayerCount will determine whether the artifact should be activated)."));
